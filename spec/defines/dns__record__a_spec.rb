...
describe 'dns::record::a', type: :define do
  ...
  context 'passing multiple ip addresses with ptr=>false' do
    ...
    it { is_expected.not_to raise_error }
    it {
      is_expected.to contain_concat__fragment('db.example.com.atest,A,example.com.record').with_content(%r{^atest\s+IN\s+A\s+192\.168\.128\.68\natest\s+IN\s+A\s+192\.168\.128\.69\natest\s+IN\s+A\s+192\.168\.128\.70$})
    }
  ...
  context 'passing multiple ip addresses with ptr=>all' do
    ...
    it {
      is_expected.to contain_concat__fragment('db.128.168.192.IN-ADDR.ARPA.69.128.168.192.IN-ADDR.ARPA,PTR,128.168.192.IN-ADDR.ARPA.record').with_content(%r{^69\s+IN\s+PTR\s+atest\.example\.com\.$})
    }
    it {
      is_expected.to contain_concat__fragment('db.128.168.192.IN-ADDR.ARPA.70.128.168.192.IN-ADDR.ARPA,PTR,128.168.192.IN-ADDR.ARPA.record').with_content(%r{^70\s+IN\s+PTR\s+atest\.example\.com\.$})
    }
  end
